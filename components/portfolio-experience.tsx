...
// COPY: Palavras-chave do Hero focadas no seu perfil Full Stack/AI
const heroHighlights = ["escalabilidade", "inteligência", "alta performance", "fluidez"];
const signaturePillars = ["Architecture", "Automation", "Performance", "Craft"];
      ...
      <div className="relative mx-auto max-w-[540px] overflow-hidden rounded-[34px] border border-[var(--border-strong)] bg-[linear-gradient(180deg,rgba(255,255,255,0.1),rgba(255,255,255,0.04))] p-4 shadow-[0_30px_120px_rgba(0,0,0,0.28)] backdrop-blur-2xl">
        ...
        <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(13,15,12,0.96),rgba(7,8,7,0.96))]">
          ...
          <div className="relative h-[560px] overflow-hidden p-6">
            ...
            {[{ label: "Frontend", top: "14%", left: "10%" }, { label: "Backend", top: "19%", right: "10%" }, { label: "AI Core", bottom: "31%", left: "12%" }, { label: "Cloud", bottom: "25%", right: "11%" }].map((item, index) => (
            ...
            <div className="absolute inset-x-6 bottom-6 space-y-4">
              <div className="rounded-[26px] border border-[var(--border-strong)] bg-white/[0.045] p-5 backdrop-blur-xl">
                <p className="text-[10px] uppercase tracking-[0.24em] text-[var(--gold-soft)]">Value Proposition</p>
                <h3 className="mt-3 font-heading text-[28px] leading-none text-[var(--text-primary)]">Engenharia de alto nível.</h3>
                <p className="mt-4 text-sm leading-7 text-[var(--text-secondary)]">
                  Construção de plataformas escaláveis que suportam operações complexas, entregues com uma interface que domina a percepção de mercado.
                </p>
                <div className="mt-5 grid grid-cols-3 gap-3">
                  {[["Uptime", "99.9%"], ["Latency", "<50ms"], ["Design", "Premium"]].map(([label, value]) => (
              ...
              <div className="rounded-[24px] border border-[var(--border-soft)] bg-[rgba(18,20,18,0.72)] p-4 backdrop-blur-xl">
                <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.24em] text-[var(--text-secondary)]">
                  <span>Tech Pillars</span>
                  <span className="text-[var(--green-secondary)]">optimized</span>
                </div>
    ...
    <Reveal delay={index * 0.08}>
      <motion.article onHoverStart={onHover} onHoverEnd={onLeave} onFocus={onHover} onBlur={onLeave} whileHover={{ y: -8, scale: 1.01 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }} data-cursor="interactive" className={`group relative flex h-full flex-col overflow-hidden rounded-[30px] border p-6 transition-[border-color,box-shadow,transform,background] duration-500 ${focused ? "border-[rgba(212,175,55,0.24)] bg-[rgba(255,255,255,0.06)] shadow-[0_24px_80px_rgba(0,0,0,0.24)]" : "border-[var(--border-soft)] bg-white/[0.035]"}`}>
          ...
          <div className="relative mt-6 flex flex-1 flex-col">
            ...
            <motion.div animate={{ height: focused ? "auto" : 0, opacity: focused ? 1 : 0 }} transition={{ duration: 0.35 }} className="overflow-hidden">
              <div className="mt-5 rounded-[22px] border border-white/8 bg-black/20 p-4">
                <p className="text-[10px] uppercase tracking-[0.22em] text-[var(--green-secondary)]">Visão Estratégica</p>
        ...
        <div className="relative mt-5 flex flex-wrap gap-3">
          <button type="button" onClick={onSelect} className="rounded-full border border-[var(--border-soft)] bg-white/[0.04] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-primary)] transition-colors hover:bg-white/[0.08]" data-cursor="interactive">Estudo de Caso</button>
          {project.url && (
            <Link href={project.url} target="_blank" rel="noreferrer" className="rounded-full border border-[rgba(212,175,55,0.22)] bg-[rgba(212,175,55,0.08)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--gold-soft)] transition-colors hover:bg-[rgba(212,175,55,0.14)]" data-cursor="interactive">Acessar Sistema</Link>
          )}
        </div>
      </motion.article>
    </Reveal>
    ...
    <AnimatePresence>
      {project ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[rgba(4,4,4,0.58)] p-4 backdrop-blur-lg">
          <motion.button type="button" className="absolute inset-0 w-full h-full" onClick={onClose} aria-label="Fechar projeto" />
          <motion.div initial={{ y: 36, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 24, opacity: 0, scale: 0.98 }} transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }} className="relative z-10 flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-[36px] border border-[var(--border-strong)] bg-[linear-gradient(180deg,rgba(10,12,10,0.98),rgba(5,5,5,0.98))] shadow-[0_30px_160px_rgba(0,0,0,0.45)]">
            <div className="flex items-center justify-between border-b border-white/8 px-6 py-5 md:px-8">
              ...
              <div className="flex items-center gap-3">
                {project.url && (
                  <Link href={project.url} target="_blank" rel="noreferrer" className="rounded-full border border-[rgba(212,175,55,0.22)] bg-[rgba(212,175,55,0.08)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--gold-soft)] hover:bg-[rgba(212,175,55,0.15)] transition-colors" data-cursor="interactive">Acessar Sistema</Link>
                )}
                <button type="button" onClick={onClose} className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-secondary)] transition-colors hover:text-[var(--text-primary)] hover:bg-white/[0.08]" data-cursor="interactive">Fechar</button>
              </div>
            </div>
            ...
            <div className="overflow-y-auto px-6 py-6 md:px-8 md:py-8">
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 md:grid-cols-1">
                      {project.metrics.map((metric) => (
                        <div key={metric} className="rounded-[20px] border border-white/8 bg-white/[0.03] p-4">
                          <p className="text-[10px] uppercase tracking-[0.22em] text-[var(--text-secondary)]">Resultado Core</p>
              ...
              <div className="mt-8 grid gap-6 lg:grid-cols-2">
                {[["Contexto & Desafio", project.challenge], ["Arquitetura & Solução", project.solution]].map(([title, text]) => (
                ...
                <div className="rounded-[26px] border border-white/8 bg-white/[0.03] p-6">
                  <p className="text-[10px] uppercase tracking-[0.22em] text-[var(--gold-soft)]">Stack de Engenharia</p>
                ...
                <div className="rounded-[26px] border border-white/8 bg-white/[0.03] p-6">
                  <p className="text-[10px] uppercase tracking-[0.22em] text-[var(--gold-soft)]">Impacto Mensurável</p>
                  <div className="mt-4 space-y-3">
                    {project.impact.map((item) => (
                      <div key={item} className="rounded-[18px] border border-white/8 bg-black/30 px-4 py-3 text-sm text-[var(--text-secondary)]">
                        <span className="text-[var(--green-secondary)] mr-2">✓</span>{item}
                      </div>
                    ))}
                  </div>
                </div>
    ...
    <Reveal>
      <motion.div onHoverStart={onHover} onHoverEnd={onLeave} onFocus={onHover} onBlur={onLeave} whileHover={{ y: -6 }} data-cursor="interactive" className={`group rounded-[26px] border p-5 transition-all duration-400 ${focused ? "border-[rgba(0,194,122,0.24)] bg-[rgba(255,255,255,0.06)] shadow-[0_18px_60px_rgba(0,0,0,0.2)]" : "border-[var(--border-soft)] bg-white/[0.035]"}`}>
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="rounded-full border border-white/8 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-[var(--text-secondary)]">Tier {item.level >= 90 ? 'S' : 'A'}</span>
        </div>
...
export function PortfolioExperience() {
  ...
  // Se você não tiver o campo "type" na sua lista nova, pode ajustar ou remover o filtro
  const systems = projects.filter((project) => project.category.toLowerCase().includes("saas") || project.category.toLowerCase().includes("software") || project.category.toLowerCase().includes("gestão"));
  const websites = projects.filter((project) => !systems.includes(project));
      ...
      >
        ...
        <section id="home" className="relative z-20 mx-auto grid min-h-[calc(100vh-76px)] w-full max-w-7xl items-center gap-14 px-6 pb-24 pt-10 md:grid-cols-[1.04fr_0.96fr] md:px-10 md:pb-28">
          <motion.div style={{ y: heroY, scale: heroScale }} className="relative">
            <Reveal><SectionLabel>Engenharia Full Stack & Automação</SectionLabel></Reveal>
            <Reveal delay={0.06}>
              <motion.h1 style={{ x: heroOffsetX, y: heroOffsetY }} className="mt-8 max-w-5xl font-heading text-5xl font-semibold leading-[0.94] tracking-[-0.055em] text-[var(--text-primary)] md:text-[5.3rem] lg:text-[6.1rem]">
                ...
                <span className="block text-[var(--text-primary)]">presença, precisão e</span>
            ...
            <Reveal delay={0.12}><p className="mt-8 max-w-2xl text-base leading-8 text-[var(--text-secondary)] md:text-lg">Desenvolvo arquiteturas completas — do front-end premium ao back-end impulsionado por IA — garantindo que o seu produto escale e domine o mercado desde o primeiro clique.</p></Reveal>
            <Reveal delay={0.18} className="mt-10 flex flex-wrap items-center gap-4">
              <MagneticButton href="#projetos">Explorar Cases</MagneticButton>
              <MagneticButton href="#contato" variant="secondary">Iniciar Projeto</MagneticButton>
            </Reveal>
            <Reveal delay={0.24} className="mt-14 grid max-w-3xl gap-4 sm:grid-cols-3">
              {[["Full Stack", "next.js + node"], ["Operação", "automação de IA"], ["Entrega", "SaaS em escala"]].map(([label, value]) => (
        ...
        <section id="sobre" className="relative z-20 mx-auto max-w-7xl px-6 py-24 md:px-10">
          <Reveal><SectionLabel>Visão Executiva</SectionLabel></Reveal>
          <div className="mt-10 grid gap-10 lg:grid-cols-[1.15fr_0.85fr]">
            <Reveal delay={0.08}>
              <h2 className="max-w-4xl font-heading text-4xl leading-[1.02] tracking-[-0.05em] text-[var(--text-primary)] md:text-[4.2rem]">
                ...
                <span className="block text-[var(--text-secondary)]">Trata-se de construir ativos digitais inquestionáveis.</span>
              </h2>
              <p className="mt-7 max-w-2xl text-base leading-8 text-[var(--text-secondary)]">Com 5 anos de experiência projetando SaaS escaláveis e integrações complexas, minha atuação conecta engenharia de ponta, automação de dados e design focado em retenção. O resultado são sistemas que suportam o crescimento do seu negócio corporativo.</p>
            </Reveal>
        ...
        <section id="projetos" className="relative z-20 mx-auto max-w-7xl px-6 py-24 md:px-10">
          <Reveal><SectionLabel>Portfólio de Engenharia</SectionLabel></Reveal>
          <Reveal delay={0.08}>
            <div className="mt-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <h2 className="max-w-3xl font-heading text-4xl leading-[1.02] tracking-[-0.05em] text-[var(--text-primary)] md:text-[4.4rem]">Engenharia aplicada a modelos de negócio de alto ticket.</h2>
              <p className="max-w-xl text-sm leading-7 text-[var(--text-secondary)]">De plataformas corporativas a e-commerces B2B. Cada projeto comprova que uma arquitetura tecnológica robusta e uma estética premium devem caminhar juntas.</p>
            </div>
          </Reveal>

          <ProjectsBlock title="Sistemas & SaaS" description="Arquiteturas complexas, dashboards executivos e plataformas com foco em segurança, integração de IA e escala operacional." items={systems} focusedSurface={focusedSurface} setFocusedSurface={setFocusedSurface} setSelectedProject={setSelectedProject} />
          <ProjectsBlock title="Conversão & Landing" description="Presenças digitais de ultra-performance otimizadas obsessivamente para captação de leads, SEO e posicionamento premium de mercado." items={websites} focusedSurface={focusedSurface} setFocusedSurface={setFocusedSurface} setSelectedProject={setSelectedProject} />
        </section>

        <section id="stack" className="relative z-20 mx-auto max-w-7xl px-6 py-24 md:px-10">
          <Reveal><SectionLabel>Arquitetura de Sistemas</SectionLabel></Reveal>
          <Reveal delay={0.08}>
            <div className="mt-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <h2 className="max-w-3xl font-heading text-4xl leading-[1.02] tracking-[-0.05em] text-[var(--text-primary)] md:text-[4.1rem]">Um ecossistema tecnológico desenhado para escala.</h2>
              <p className="max-w-xl text-sm leading-7 text-[var(--text-secondary)]">Minha stack prioriza segurança corporativa e velocidade de iteração, dominando o ambiente JavaScript moderno e orquestração de APIs externas.</p>
            </div>
          </Reveal>
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-[38px] border border-[var(--border-strong)] bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] p-8 shadow-[0_26px_100px_rgba(0,0,0,0.42)] md:p-12">
              ...
              <div className="relative">
                <SectionLabel>Início de Parceria</SectionLabel>
                <div className="mt-8 grid gap-10 md:grid-cols-[1fr_auto] md:items-end">
                  <div>
                    <h2 className="max-w-3xl font-heading text-4xl leading-[1.02] tracking-[-0.05em] text-[var(--text-primary)] md:text-[4rem]">Pronto para elevar o patamar tecnológico da sua operação?</h2>
                    <p className="mt-6 max-w-2xl text-base leading-8 text-[var(--text-secondary)]">Disponível para construir produtos seletivos: de plataformas SaaS do zero a modernização de sistemas legados com inteligência artificial e interfaces de alto padrão.</p>
                  </div>
                  <div className="flex min-w-[240px] flex-col gap-3">
                    <Link href="https://wa.me/5511942716215" data-cursor="interactive" className="rounded-full border border-[rgba(212,175,55,0.22)] bg-[linear-gradient(135deg,rgba(241,209,122,0.96),rgba(212,175,55,0.9))] px-6 py-3 text-center text-sm font-semibold text-[#11110d] shadow-[0_12px_40px_rgba(212,175,55,0.2)] transition-transform duration-300 hover:scale-[1.015] active:scale-[0.985]">Conversar no WhatsApp</Link>
                    <Link href="mailto:pedrooofreitas@gmail.com" data-cursor="interactive" className="rounded-full border border-[var(--border-soft)] bg-white/[0.03] px-6 py-3 text-center text-sm font-semibold text-[var(--text-primary)] transition-colors duration-300 hover:bg-white/[0.05]">Enviar e-mail de briefing</Link>
                  </div>
                </div>
              </div>
            </div>
